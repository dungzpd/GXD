...
class UserController extends BaseController {
    ...
    public function index() {      
        ...
        return view('Backend::user.index', compact('users', 'keyword', 'field', 'sort'));
    }
    
    public function create(Request $request) {
        $result = array('status' => false, 'action' => 'create', 'title' => 'user.accountCreate', 'breadcrumbs' => 'account.create', 'actionForm' => URL::action('\Backend\Controllers\UserController@create'), 'data' => '', 'messages' => array('success' => '', 'errors' => ''));
        if ($request->isMethod('POST')) {
            $params = Input::all();
            $result = $this->doCreate($params, $result);
            if($result['status'] !== false) {
                if (isset($params['saveAndCreate'])) {
                    return redirect()->action('\Backend\Controllers\UserController@create');
                }
